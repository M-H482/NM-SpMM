...
void PreProcessing(int* column_info, int pruning_M, int pruning_N, int* DT, int W, int Q, int Ns)
{
    // re indexing
    int Qs = Ns / alignN;
...
__global__ void nmGEMM(float* A, float* B, int* D, int* column_info, float* C, int M, int N, int K, int W)
{
    ...
    int* col_info = Ds + Ws * Qs * sizeof(int);
    ...
    for (int u = 0, v = 0; u < W; u += Ws, v += Ks) {
        ...
        int* column_info_ptr = column_info + blockIdx.x * (W / Ws) * Ks + v;

        if (tid < Ks) {
            col_info[tid] = column_info_ptr[tid];
        }
        __syncthreads();

#pragma unroll
        for (int i = 0; i < Ks; i += A_STRIDES) {
            if (col_info[i + A_BLOCK_ROW_START] != -1) {
                FETCH_FLOAT4(As[(i + A_BLOCK_ROW_START) * Ms + A_BLOCK_COL_START])
                    = FETCH_FLOAT4(A_ptr[col_info[i + A_BLOCK_ROW_START] * M + A_BLOCK_COL_START]);
            }
        }
#pragma unroll
...
void nmspmm(float* A, float* B, int* D, int* column_info, float* C, int M, int N, int K, int W, float sparsity)
{
    const int Ms = 64;
    const int Ns = 128;
    const int Mt = 8;
...
int main(int argc, char** argv)
{
    ...
    PreProcessing(column_info, pruning_M, pruning_N, hD_T, W, Q, Ns);
    int cnt = 0;
    ...
    // nmspmm(dA, dB, dD, dcolumn_info, dC, M, N, K, W, sparsity);
