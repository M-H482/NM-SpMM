...
template <
    ...
    const int Mt,
    const int Nt>
__global__ void kernel_32x64_8x4_low_sparsity(float* A, float* B, int* D, float* C, int M, int N, int K, int W)
{
    /*
    ...
    const int ti = ((lane_id / 16) * 2 + (lane_id % 2)) * 4;
    const int tj = warp_id * 32 + ((lane_id / 2) % 8) * 4;
...
#pragma unroll
    ...
    FETCH_FLOAT4(At[0][4]) = FETCH_FLOAT4(As_write_ptr[idx[0] * Ms + ti + 16]);
...
#pragma unroll
        for (int p = 0; p < Ws - 1; p += 1) {
            ...
            FETCH_FLOAT4(At[(p + 1) % 2][4]) = FETCH_FLOAT4(As_read_ptr[idx[p + 1] * Ms + ti + 16]);
#pragma unroll
            for (int i = 0; i < Mt; i++) {
                if (i % 2) {
#pragma unroll
...
#pragma unroll
        for (int i = 0; i < 4; i++) {
            ...
            atomicAddFloat4(&C[(bi + ti + i + 16) * N + (bj + tj + 0)], &Ct[i + 4][0]);
        }
    } else {
#pragma unroll
        for (int i = 0; i < 4; i++) {
            ...
            FETCH_FLOAT4(C[(bi + ti + i + 16) * N + (bj + tj + 0)]) = FETCH_FLOAT4(Ct[i + 4][0]);
        }
    }
}
...
void nmGEMM_medium_matrices_low_sparsity(float* A, float* B, int* D, float* C, int M, int N, int K, int W, float sparsity, int SPLIT_K)
{
    ...
    if (fabs(sparsity - 0.75f) < 1e-6 || fabs(sparsity - 0.875f) < 1e-6) {
        ...
        kernel_32x64_8x4_low_sparsity<Ms, Ns, Ks, Ws, Mt, Nt>
...
template <
    ...
    const int Mt,
    const int Nt>
__global__ void kernel_32x64_8x4_high_sparsity(float* A, float* B, int* D, int* column_info, float* C, int M, int N, int K, int W)
{
    /*
     *    A, B, D, C: col-major, row-major, row-major, row-major
     */
...
void nmGEMM_medium_matrices_high_sparsity(float* A, float* B, int* D, int* column_info, float* C, int M, int N, int K, int W, float sparsity, int SPLIT_K)
{
    ...
    if (fabs(sparsity - 0.5f) < 1e-6 || fabs(sparsity - 0.625f) < 1e-6) {
        ...
        size_t smem_nbytes = 2 * (Ks * Ms + Ws * Ns) * sizeof(float)
            + 2 * (Ws * Ns / VEC_LEN + Ks) * sizeof(int);
        cudaFuncSetAttribute(kernel_32x64_8x4_high_sparsity<Ms, Ns, Ks, Ws, Mt, Nt>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_nbytes);
        kernel_32x64_8x4_high_sparsity<Ms, Ns, Ks, Ws, Mt, Nt>
